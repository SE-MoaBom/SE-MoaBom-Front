...
const SchedulerPage: React.FC = () => {
  ...
  // 중복 제거 헬퍼 함수 (programId 기준)
  const getUniquePrograms = (programs: Program[]) => {
    return Array.from(new Map(programs.map((p) => [p.programId, p])).values());
  };
      ...
      <main className="scheduler-container">
        ...
        <section className="plan-summary">
          ...
          <p className="subtitle">
            ...
            {scheduleData.actions.reduce(
              (sum: number, plan: PlanAction) =>
                sum + getUniquePrograms(plan.programs).length, // 여기도 중복 제거된 개수로 계산
              0
            )}
        ...
        <section className="timeline-section">
          ...
          >
            ...
            {/* 플랜 블록 */}
            {scheduleData.actions.map((plan: PlanAction, index: number) => {
              const uniquePrograms = getUniquePrograms(plan.programs); // 중복 제거
              return (
                <div
            ...
            return (
              <div className="accordion-item" key={plan.ottName}>
                ...
                >
                  ...
                  <p>
                    <strong>시청 목록:</strong>{" "}
                    {uniquePrograms.map((p) => p.title).join(", ")}
                  </p>
                </div>
              </div>
            );
